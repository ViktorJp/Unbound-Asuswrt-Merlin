...
Show_status() {
    ...
    if [ "$(Unbound_Installed)" == "Y" ];then
        ...
        if [ -n "$(grep "^outgoing-interface" ${CONFIG_DIR}unbound.conf)" ];then   # v3.16
            # Use obscure but benign URLthat most will never reference..... Changed to zombo.com on Nov-03-2025 due to amdahl.com going down @ViktorJp
            if [ -z "$(nslookup zombo.com | grep -woE '([0-9]{1,3}\.){3}[0-9]{1,3}' | awk 'NR>2')" ];then
            ...
            # .....Remove it from the cache to prevent a false-positive for next time... Changed to zombo.com on Nov-03-2025 due to amdahl.com going down @ViktorJp
            $UNBOUNCTRLCMD flush zombo.com 1>/dev/null   # v3.16
        fi
    fi
}
